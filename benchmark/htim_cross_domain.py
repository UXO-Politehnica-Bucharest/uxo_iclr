#!/usr/bin/env python3
"""H-TIM (TIM-GD's fixed hyperparameters on the hyperbolic manifold, see
``htim_ctx_uxo.py``) on the same cross-domain episodes as
``cross_domain_eval.py``.

Usage:
    python3 benchmark/htim_cross_domain.py --device cuda \\
        --dataset fgvc_aircraft --backbone dinov3 --num-episodes 100 \\
        --shots 1,3,5,10 --output-dir results_run/htim_crossdomain_fgvc_aircraft_dinov3
"""
...
def main() -> None:
    ...
    print(f"[H-TIM] Dataset: {args.dataset}   Backbone: {args.backbone}   Device: {args.device}", flush=True)
    ...
    print(f"Diagnostic-calibrated d_eff = {d_eff} (K fixed at {K_CURVATURE}). "
          f"H-TIM: T=1000, lr=1e-4, tau=7.5, hyperbolic -- no search.", flush=True)
