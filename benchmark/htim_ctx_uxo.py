#!/usr/bin/env python3
"""H-TIM on CTX-UXO: TIM-GD's fixed hyperparameters (T=1000, lr=1e-4,
tau=7.5, loss weights [0.1, 1.0, 0.1], query-only MI pool; see
``benchmark.baselines._euclidean_tim_original_predict``) run on the
hyperbolic manifold with ``algorithm.rtim.htim_adapt`` (R-Adam). Only the
geometry differs from TIM (Euclidean, original); no hyperparameter search.

Uses the same paired test episodes as ``export_full_results.py``.

Usage:
    python3 benchmark/htim_ctx_uxo.py --device cuda --backbone dinov3 \\
        --num-episodes 100 --shots 1,3,5,10 --output-dir results_run/htim_table3_dinov3
"""
...
from algorithm.rtim import HTIMConfig, cl2n_condition, fit_pca_projection, hyperbolic_lift, htim_adapt
...
METHOD_NAME: str = "H-TIM"

# Same fixed constants as TIM-original in benchmark/baselines.py.
_T: int = 1000
_BETA: float = 1e-4
_TAU: float = 7.5
_CE_WEIGHT: float = 0.1
_MARGINAL_H_WEIGHT: float = 1.0
_COND_H_WEIGHT: float = 0.1


def _htim_config(K: float, d_eff: int) -> HTIMConfig:
    return HTIMConfig(
        K=K, d_eff=d_eff, T=_T, beta=_BETA, tau=_TAU, omega=0.0,
        use_query_in_mi=False,  # TIM-GD's own query-only pool
        ce_weight=_CE_WEIGHT, marginal_h_weight=_MARGINAL_H_WEIGHT, cond_h_weight=_COND_H_WEIGHT,
        optimizer="radam",
    )
...
def main() -> None:
    ...
    print(f"[H-TIM] Generating {args.num_episodes} paired episodes per shot for S in {shots} "
    ...
    print(f"d_eff = {d_eff} (K={K_CURVATURE}, fixed H-TIM hyperparameters -- no search)", flush=True)
    config = _htim_config(K_CURVATURE, d_eff)
    ...
    for shot in shots:
        ...
        print(f"\nS={shot}: evaluating {len(episodes)} episodes with H-TIM (T=1000, lr=1e-4, tau=7.5, "
              f"hyperbolic) ...", flush=True)
